...
// const API_HOST = "https://moviebooking-yqod.onrender.com";
...
// admin/src/components/AddPage/AddMoviePage.jsx
...
export default function AddMoviePage() {
  ...
  const uploadToCloudinary = async (file) => {
    ...
    const res = await fetch(
      "https://api.cloudinary.com/v1_1/dummyvalue/image/upload",
      {
        method: "POST",
        body: data,
      }
  ...
  const getYoutubeThumbnail = (url) => {
    try {
      const reg = /(?:youtube\.com.*(?:v=|embed\/)|youtu\.be\/)([^&?/]+)/;
  ...
  const handleMultiImageSelect = (e, setFunc) => {
  ...
  const removeImage = (index, setFunc) => {
  ...
  const handleSubmit = async () => {
    try {
      if (!movieName || !poster) {
        return toast.error("Movie name & Poster are required");
      }

      toast.info("Uploading poster...");

      // Upload poster first
      const posterUrl = await uploadToCloudinary(poster);

      // Upload cast/director/producer images
      const uploadMulti = async (files) => {
        const uploads = await Promise.all(
          files.map((img) => uploadToCloudinary(img))
        );
        return uploads;
      };

      const castUrls = await uploadMulti(castImages);
      const directorUrls = await uploadMulti(directorImages);
      const producerUrls = await uploadMulti(producerImages);

      // Trailer auto thumbnail
      const trailerThumbnail = getYoutubeThumbnail(trailerUrl);
      ...
      const payload = {
        ...
        images: {
          cast: castUrls,
          director: directorUrls,
          producer: producerUrls,
        },
        releaseInfo,
      };

      toast.info("Uploading movie...");

      const res = await axios.post(`${API_HOST}/api/movies/create`, payload);
  ...
  const resetForm = () => {
    ...
    setDuration(120);
    setSlots([]);
    setReleaseInfo({ id: Date.now(), date: "", time: "" });
    setCastImages([]);
    setDirectorImages([]);
    setProducerImages([]);
    setStory("");
  ...
  // ------------------------------------
  return (
    <div className={addMoviePageStyles.container}>
      <ToastContainer />

      <h1 className={addMoviePageStyles.title}>Add New Movie</h1>

      <div className={addMoviePageStyles.form}>
        ...
        {posterPreview && (
          <img
            src={posterPreview}
            alt="poster preview"
            className={addMoviePageStyles.posterPreview}
          />
        )}
        ...
        <label>
          ...
          <input
            ...
            onChange={(e) =>
              setReleaseInfo({ ...releaseInfo, date: e.target.value })
            }
            className={addMoviePageStyles.input}
          />
        </label>
        ...
        <label>
          ...
          <input
            type="time"
            value={releaseInfo.time}
            onChange={(e) =>
              setReleaseInfo({ ...releaseInfo, time: e.target.value })
            }
            className={addMoviePageStyles.input}
          />
        </label>
        ...
        <label>
          ...
          <input
            ...
            multiple
            onChange={(e) =>
              handleMultiImageSelect(e, setCastImages)
            }
          />
        </label>

        {/* Display selected cast images */}
        <div className={addMoviePageStyles.multiImagePreview}>
          {castImages.map((img, i) => (
            <div key={i} className={addMoviePageStyles.previewBox}>
              <img src={URL.createObjectURL(img)} />
              <button onClick={() => removeImage(i, setCastImages)}>
                <X />
              </button>
            </div>
          ))}
        ...
        <label>
          ...
          <input
            ...
            multiple
            onChange={(e) =>
              handleMultiImageSelect(e, setDirectorImages)
            }
          />
        </label>

        <div className={addMoviePageStyles.multiImagePreview}>
          {directorImages.map((img, i) => (
            <div key={i} className={addMoviePageStyles.previewBox}>
              <img src={URL.createObjectURL(img)} />
              <button onClick={() => removeImage(i, setDirectorImages)}>
                <X />
              </button>
            </div>
          ))}
        ...
        <label>
          ...
          <input
            ...
            multiple
            onChange={(e) =>
              handleMultiImageSelect(e, setProducerImages)
            }
          />
        </label>

        <div className={addMoviePageStyles.multiImagePreview}>
          {producerImages.map((img, i) => (
            <div key={i} className={addMoviePageStyles.previewBox}>
              <img src={URL.createObjectURL(img)} />
              <button onClick={() => removeImage(i, setProducerImages)}>
                <X />
              </button>
            </div>
          ))}
        ...
        {/* Submit */}
        <button
          className={addMoviePageStyles.submitBtn}
