...
/**
 * Accepts a variety of slot shapes:
 * - { date: "YYYY-MM-DD", time: "13:00", ampm: "PM" }
 * - { datetime: "2025-12-07T13:00:00+05:30" }
 * - ISO string
...
function slotToISO(slot) {
  ...
  if (typeof slot === "object") {
    ...
    // handle if datetime exists (ISO)
    if (slot.datetime) return slot.datetime;
    if (slot.iso) return slot.iso;
    // legacy: object with time only -> not enough info
  }
  return null;
}
...
function normalizeMovieForClient(it = {}) {
  ...
  // producer shortcut
  if (!item.producer && Array.isArray(item.producers) && item.producers.length) {
    item.producer = item.producers[0];
  }
  ...
  item.slots = Array.isArray(item.slots) ? item.slots : item.slots ? [item.slots] : [];
...
export default function MovieDetailPage() {
  ...
  const [movie, setMovie] = useState(initialFromStateMovie ? normalizeMovieForClient(initialFromStateMovie) : null);
    ...
    (async function fetchMovie() {
      ...
      try {
        ...
        if (!data || data.success === false) {
          ...
          const itemRaw = data.item || data.data || (data.success ? data : null);
  ...
  const showtimeDays = useMemo(() => {
    ...
    const rawSlots =
      (Array.isArray(movie.slots) && movie.slots.length && movie.slots) ||
      (Array.isArray(movie.showtimes) && movie.showtimes.length && movie.showtimes) ||
      (Array.isArray(movie._normalizedSlots) && movie._normalizedSlots.length && movie._normalizedSlots) ||
      [];
    ...
    rawSlots.forEach((raw) => {
      try {
        ...
        const dateKey = formatDateKey(d, TZ);
        if (!slotsByDate[dateKey]) slotsByDate[dateKey] = [];
        const audi = (raw && raw.audi) || (raw && raw.auditorium) || raw.auditorium || "Audi 1";
        slotsByDate[dateKey].push({ iso, audi });
      } catch (e) {
  ...
  const buildSeatSelectorPath = (movieIdLocal, datetime) => {
    const key = encodeURIComponent(datetime);
    const usesSingular = (location.pathname || "").toLowerCase().includes("/movie/");
    const mid = movie._id || movie.id || movieIdLocal || movieIdParam;
    return usesSingular ? `/movie/${mid}/seat-selector/${key}` : `/movies/${mid}/seat-selector/${key}`;
  };
  const handleTimeSelect = (datetime) => {
    setSelectedTime(datetime);
  ...
  const posterSrc =
    ...
    movie.poster ||
    movie.posterUrl ||
    (movie.thumbnail && typeof movie.thumbnail === "string" && movie.thumbnail.startsWith("http")
      ? movie.thumbnail
      : `${API_BASE}/uploads/placeholder.png`);
  const categoryList = Array.isArray(movie.categories)
    ? movie.categories
    ...
    ? [movie.genre]
    : [];
  const producer = movie.producer || (Array.isArray(movie.producers) && movie.producers[0]) || null;
  const producerImg = producer ? producer.img || producer.preview || (producer.file ? getImageUrl(producer.file) : null) : null;
  ...
  return (
    <div className={movieDetailHStyles.pageContainer}>
      ...
      <div className={movieDetailHStyles.mainContainer}>
        ...
        <div className={movieDetailHStyles.titleContainer}>
          <h1 className={movieDetailHStyles.movieTitle} style={{ fontFamily: "'Cinzel', 'Times New Roman', serif", textShadow: "0 4px 20px rgba(220, 38, 38, 0.6)", letterSpacing: "0.08em" }}>
            {movie.title || movie.movieName || "Untitled"}
          </h1>
          <div className={movieDetailHStyles.movieInfoContainer}>
            <span className={movieDetailHStyles.rating}>
            ...
            <span className={movieDetailHStyles.genre}>{categoryList.length ? categoryList.join(", ") : "—"}</span>
          </div>
        </div>
                ...
                {showtimeDays.length ? (
                  showtimeDays.map((day, index) => (
                    <button
                      ...
                      className={`${movieDetailHStyles.dayButton} ${selectedDay === index ? movieDetailHStyles.dayButtonSelected : movieDetailHStyles.dayButtonDefault}`}
                      aria-pressed={selectedDay === index}
                      aria-label={`Select ${day.dayName} ${day.dateStr}`}
                    >
                  ...
                  showtimeDays[selectedDay].showtimes.map((showtime, index) => {
                    ...
                    return (
                      <button
                        key={index}
                        onClick={() => handleTimeSelect(showtime.datetime)}
                        className={`${movieDetailHStyles.showtimeButton} ${selectedTime === showtime.datetime ? movieDetailHStyles.showtimeButtonSelected : movieDetailHStyles.showtimeButtonDefault}`}
        ...
        <div className={movieDetailHStyles.storyCard}>
          ...
          <p className={movieDetailHStyles.storyText}>{movie.story || movie.description || movie.synopsis}</p>
        </div>
        ...
        <div className={movieDetailHStyles.crewGrid}>
          <div className={movieDetailHStyles.crewCard}>
            ...
            <div className={movieDetailHStyles.crewContent}>
              {(() => {
                const directors = Array.isArray(movie.directors) ? movie.directors : movie.director ? [movie.director] : [];
                return (
                  <div className={movieDetailHStyles.crewGridInner}>
                    {directors.length ? (
                      directors.slice(0, 2).map((d, i) => (
          ...
          <div className={movieDetailHStyles.crewCard}>
            ...
            <div className={movieDetailHStyles.crewContent}>
              ...
              <div className={movieDetailHStyles.crewName}>{producer?.name ?? "N/A"}</div>
            </div>
          </div>
        </div>
      </div>
