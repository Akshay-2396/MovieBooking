...
export default function MovieDetailPage() {
  ...
  // If navigation passed the movie object, use it to avoid refetch
  ...
  useEffect(() => {
    // If data already present from route state, skip API call
    ...
    async function fetchMovie() {
      try {
        ...
        const movieData = data.item || data;
  ...
  if (loading) return <div className="p-8 text-gray-300">Loading movie...</div>;
  if (error) return <div className="p-8 text-red-400">{error}</div>;
  if (!movie) return <div className="p-8 text-gray-300">Movie not found.</div>;
  ...
  const poster =
    movie.poster ||
    movie.latestTrailer?.thumbnail ||
    movie.thumbnail ||
    "https://placehold.co/400x600?text=No+Poster";

  // Slots (showtimes) from DB
  const showTimes = Array.isArray(movie.slots) ? movie.slots : [];
          ...
          {showTimes.length === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-4">
              {showTimes.map((slot, index) => (
                ...
                >
                  <div className="font-semibold">{slot.auditorium}</div>
                  <div className="mt-1">
